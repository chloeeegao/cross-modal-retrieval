...
def config():
    exp_name = "vilt"
    seed = 0
    datasets = ["coco", "vg", "sbu", "gcc", "recipe1m"]
    ...
    data_root ='/data/s2478846/data'
    log_dir = "result"
    per_gpu_batchsize = 0  # you should define this manually with per_gpu_batch_size=#
    num_gpus = 2
    num_nodes = 1
    load_path = '' # "/data/s2478846/ViLT/weights/vilt_irtr_coco.ckpt"
    num_workers = 16
    precision = 16
...
@ex.named_config
def task_mlm_itm_randaug_recipe():
    exp_name = "mlm_itm_randaug_recipe"
    datasets = ["recipe1m"]
    train_transform_keys = ["pixelbert_randaug"]
    loss_names = _loss_names({"itm": 1, "mlm": 1})
    batch_size = 4096
...
@ex.named_config
def task_finetune_recipe1m_randaug():

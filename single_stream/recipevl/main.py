...
def main(rank, args, world_size):
    ...
    if args.do_train:
        loss_names = {'itm': 1, 'mlm': 1, 'irtr': 0.5}
    else:
        loss_names = {'itm': 1, 'mlm': 0.5, 'irtr': 1}
    model = RecipeVL(args, loss_names)
    
    if args.do_train:
        args.draw_false_text = 0
        ...
        train_dataset = get_dataset(args, tokenizer, split='val', mode='val', sample='all')
        test_dataset = get_dataset(args, tokenizer, split='test', mode='val', sample=args.eval_sample)
        trainer = Trainer(args, model, train_dataset, test_dataset, rank, is_train=True)
        trainer.train()

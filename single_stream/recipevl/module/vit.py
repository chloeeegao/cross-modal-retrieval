...
from timm.models.layers import trunc_normal_, DropPath, to_2tuple, StdConv2dSame
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        """
        super().__init__()
        drop_rate = drop_rate if config is None else getattr(config, "hidden_dropout_prob")
    ...
    def visual_embed(self, _x):
        ...
        x = self.patch_embed(_x) # shape [b, 768, 16, 16]
        
        # x_mask = (_x.sum(dim=1) != 0).float()[:, None, :, :] # shape [b, 1, 225, 224]
        # x_mask = F.interpolate(x_mask, size=(x.shape[2], x.shape[3])).long() # shape [b, 1, 16, 16]

        B, C, H, W = x.shape
        
        # pos_embed = pos_embed.flatten(2).transpose(1, 2)
        x = x.flatten(2).transpose(1, 2)  # shape: [4, 256, 768]
        # patch_index = (
        #     torch.stack(
        #         torch.meshgrid(
        ...
        # x_mask = x_mask.flatten(1) # shape [b, 256]
        
        x = x.view(B, -1, C)
        # x_mask = x_mask.view(B, -1)
        # patch_index = patch_index.view(B, -1, 2)
        # pos_embed = pos_embed.view(B, -1, C)
        ...
        x_mask = torch.ones(x.size()[:-1], dtype=torch.long).to(x.device)
        
        return x, x_mask 

    def forward_features(self, _x):
        x, x_mask = self.visual_embed(_x)
